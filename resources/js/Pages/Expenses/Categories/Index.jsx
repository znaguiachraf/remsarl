...
export default function ExpenseCategoriesIndex({ project, categories }) {
    ...
    const handleCreate = (e) => {
        e.preventDefault();
        createForm.post(route('projects.modules.expenses.categories.store', project.id), {
            preserveScroll: true,
            onSuccess: () => {
                createForm.reset();
    ...
    const handleUpdate = (e) => {
        ...
        editForm.patch(route('projects.modules.expenses.categories.update', [project.id, editingCategory.id]), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    const deleteCategory = (category) => {
        if (confirm(`Delete category "${category.name}"? This cannot be undone.`)) {
            router.delete(route('projects.modules.expenses.categories.destroy', [project.id, category.id]), {
                preserveScroll: true,
            });
        }
    };
    ...
    return (
            ...
            header={
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-xl font-semibold leading-tight text-gray-800">Expense Categories</h2>
                            <p className="mt-0.5 text-sm text-gray-500">Organize expenses by category</p>
                        </div>
                    </div>
        ...
        >
            <Head title={`${project?.name} - Expense Categories`} />
            ...
            <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                    ...
                    <thead className="bg-gray-50">
                        <tr>
                            <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">
                                <span className="inline-flex items-center gap-1.5"><IconDocument className="h-4 w-4" />Name</span>
                            </th>
                            <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">
                ...
                {(!categories || categories.length === 0) && (
                    <div className="flex flex-col items-center justify-center px-6 py-12 text-center text-gray-500">
                        ...
                        <p className="mt-1 text-sm">Add your first category to organize expenses.</p>
                    </div>
                )}
            </div>
